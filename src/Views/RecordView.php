...
<body>';
    ...
    public function renderIndex(array $records, int $total, int $currentPage, int $limit): string
    {
        ...
        $html .= '<h1 class="mb-0">Записи</h1>';
        $html .= '<a href="?action=insert" class="btn btn-outline-dark">Добавить запись</a>';
        $html .= '</div>';
    ...
    public function renderForm(string $mode, array $record = []): string
    {
        ...
        $id     = $record['id'] ?? '';
        $name   = $record['name'] ?? '';
        $title  = $isEdit ? 'Редактирование записи' : 'Новая запись';
        ...
        $html .= '<div class="mb-3">';
        $html .= '<label for="name" class="form-label">Название</label>';
        $html .= '<input type="text" class="form-control" id="name" name="name" value="' . htmlspecialchars($name) . '" required autofocus>';
        $html .= '</div>';
        $html .= '<div class="d-flex gap-2 mt-4">';
        $html .= '<button type="submit" class="btn btn-outline-dark">Сохранить</button>';
        $html .= '<a href="?action=index" class="btn btn-outline-secondary">Отмена</a>';
        $html .= '</div>';
